...
with col3:
    ...
    # Show the predicted rent per m² for the selected year
    st.subheader(f"Predicted Rent per m² for {selected_year}: €{predicted_rent_per_m2:.2f}")
...
with col3:
    ...
    plt.scatter(future_years.flatten(), future_predictions, color='blue', label="Predicted Prices", zorder=5)
    plt.plot(np.append(df['Year'], future_years.flatten()), np.append(df['Price Per m2 (42.5m2)'], future_predictions), '--', color='blue', alpha=0.7)

    plt.title("Forecasted Rent per m²")
    plt.xlabel('Year')
